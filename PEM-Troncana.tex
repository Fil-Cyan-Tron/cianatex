...
\section*{Fondamenti assiomatici}
...
\begin{remark}{Sull'assioma di Universo debole}{sull'assioma di universo debole}
    ...
    \end{axiom}
    Questo implica l'esistenza di tutta una gerarchia di universi ciascuno "inaccessibile" da quello precedente, utile per trattare la Teoria delle Categorie Superiori, ma per una trattazione più elementare è sufficiente assumere l'esistenza di un solo universo, si veda \cite{MacLane1969}.
\end{remark}
...
\begin{remark}{Lemma di Yoneda e grandezza}{lemma di Yoneda e grandezza}
    ...
    Prendiamo ad esempio il funtore identità $\id$ da $\setcat$ in sè stessa: noi abbiamo che $\nat(h_S,\id)$ è in biezione con $S$, ma gli elementi di $\nat(h_S,\id)$ sono indicizzati da $\G$, che non è affatto un insieme piccolo.\\
    Avendo l'assioma \ref{ax:assioma di universo debole} non incorriamo in problemi di tipo logico o fondativo, ma se non avessimo limitato in questo modo la grandezza delle nostre categorie dovremmo gestire in qualche modo la collezione $\nat(h_S,\id)$, che per quanto ci sia garantito debba essere "grande quanto" un insieme, sarebbe una collezione di elementi classicamente considerati come classi proprie, ovvero classi che non possono appartenere ad altre classi.
\end{remark}
...
Il pullback è una sorta di analogo categoriale delle equazioni: è facile dimostrare che in $\setcat$ e nelle categorie concrete\footnote{Informalmente, quelle i cui oggetti sono insiemi dotati di una qualche struttura e i cui morfismi sono funzioni insiemistiche che rispettano tale struttura; formalmente, quelle che ammettono un funtore fedele a valori in $\setcat$.} il pullback di $\{ X\xrightarrow{f} Z \xleftarrow{g} Y \}$ è il sottoinsieme $\{ (x,y) | f(x) = g(y) \}$ di $X\times Y$ e i morfismi che lo accompagnano sono la restrizione delle proiezioni. Un altro esempio è quello del grafico di una funzione, che è il pullback del diagramma $\{ X\xrightarrow{f} Y \xleftarrow{\id_Y} Y \}$.
