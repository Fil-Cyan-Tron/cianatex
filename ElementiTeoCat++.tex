...
\title{Categories for the Learning Mathematician}
...
\chapter{Elementi di Teoria delle Categorie}
...
\begin{definition}{Oggetto esponenziale}{oggetto esponenziale}
    Sia $\C$ una categoria e siano $Z,Y$ oggetti di $\C$ tali che esista sempre il prodotto binario $-\times Y$ (e che dunque definisca un funtore $\C\to\C$).\\
    Un oggetto $Z^Y$ si dice \bemph{esponenziale} in $\C$ se è munito di un morfismo $\eval_{Y,Z}:Z^Y\times Y\to Z$ tale che per ogni morfismo  $g:X\times Y \to Z$ esista un unico $\lambda g : X\to Z^Y$ tale che il seguente diagramma commuti:
    \[\begin{tikzcd}
    	{X\times Y} \\
	    \\
	    {Z^Y\times Y} && Z
    ...
    \end{tikzcd}\]
    L'assegnazione $g\mapsto \lambda g$ definisce una biezione $\hom(X\times Y, Z)\cong \hom(X,Z^Y)$.
\end{definition}

\begin{definition}{Categoria Cartesiana Chiusa}{CCC}
    Sia $\C$ una categoria. Questa si dice \bemph{cartesiana chiusa} se:\begin{itemize}
        \item $\C$ ammette un oggetto terminale $\term$ e tutti i prodotti binari, o equivalentemente ammette tutti i prodotti finiti, dato che $\term = \prod \varnothing$.
...
\begin{lemma}{}{}
    Sia $\C$ cartesiana chiusa e localmente piccola.\\
    Definiti per ogni $B \in \ob\C$ i funtori $\Pi_B, \E_B : \C\to\C$ come:
    \[ \Pi_B: (f:X\to Y) \mapsto (f\times\id_B : X\times B\to Y \times B) \]
    \[ \E_B: (f: X\to Y)\mapsto ( f^* : X^B\to Y^B ) \]

\end{lemma}

\begin{example}{$\setcat$ è cartesiana chiusa}{}
    La categoria $\setcat$ è cartesiana chiusa.
    \proof 
    Controllando le tre condizioni:\begin{itemize}
        \item L'oggetto terminale è il singoletto $\{\varnothing\}$.
        \item Il prodotto di insiemi $X$ e $Y$ è il loro prodotto cartesiano.
        \item L'esponenziale $Z^Y$ è dato dall'insieme $\hom(Y,Z)$ e le mappe $g\mapsto \lambda g$ e $\eval_{Y,Z}$ sono definite dal seguente diagramma:
        \[\begin{tikzcd}
	        {X\times Y} \\
        	& {(x,y)} \\
	        \\
	        & {(\lambda g(x):= g(x,\bullet): Y\to Z, y)} && {\lambda g(x)(y) := g(x,y)} \\
        	{\hom(Y,Z)\times Y} &&&& Z
        	\arrow["{\lambda g\times \id_B}"{description}, from=1-1, to=5-1]
	        \arrow[maps to, from=2-2, to=4-2]
        ...
        \end{tikzcd}\]
    \end{itemize}
    Dunque $\setcat$ è cartesiana chiusa, in particolare è l'esempio prototipico di categoria cartesiana chiusa.
    \qed 
\end{example}

\begin{example}{$\veccat(\K)$ non è cartesiana chiusa}{}
    Sia $\K$ un campo e consideriamo la categoria $\veccat(\K)$; questa non è cartesiana chiusa.
    \proof 
    Per dimostrarlo usiamo un lemma:
    \begin{lemma}{}{}
        Se una categoria $\C$ con un oggetto zero $\init$ è cartesiana chiusa, allora tutti gli oggetti di $\C$ sono isomorfi tramite un unico isomorfismo.
        \proof 
        Dato che $\init$ è sia iniziale che terminale, per ogni coppia di oggetti $X,Y$ abbiamo:
        \[ \hom(X,Y)\cong \hom(X\times \init, Y) \cong \hom(\init, Y^X ) \cong \{\varnothing\} \]
        E naturalmente vale lo stesso per $\hom(Y,X)$, dunque $X$ è isomorfo a $Y$ tramite un unico isomorfismo.
    \end{lemma}
    Notiamo che per qualsiasi campo $\K$ lo spazio vettoriale banale $\init := \{0_\init\}$ è l'oggetto zero di $\veccat \K$, in quanto per ogni spazio $V$ abbiamo le uniche mappe $0_\init\mapsto 0_V$ e $v\mapsto 0_\init$.\\
    Siccome ha un oggetto zero, se $\veccat\K$ fosse cartesiana chiusa tutti gli spazi vettoriali sarebbero isomorfi, assurdo, dunque otteniamo la tesi.
    \qed 
\end{example}



\newcommand\PER{\catname{PER}}

\chapter{La categoria $\PER(\N) $}

In questa parte consideremo un esempio non banale di categoria cartesiana chiusa costruita in CITA, la categoria $\PER(\N)$, i cui oggetti sono le relazioni di equivalenza parziali sui numeri naturali con dei morfismi opportunamente definiti in termini di computabilità.

\section{Funzioni parziali ricorsive}

Definiamo una classe di funzioni 

\renewcommand\R{\mc{R}}
\begin{definition}{Funzioni parziali ricorsive}{}
    Consideriamo su un sottoinsieme $R\subset P := \{f: \N^n\xrightarrow{\text{parz}} \N : n \in \N\}$ le seguenti condizioni:\begin{enumerate}
        \item $R$ contiene le funzioni costantemente nulle.
        \item $R$ contiene la funzione successore $n\mapsto n+1$.
        \item Per ogni $n$ in $\N$, $R$ contiene le proiezioni $\pi_i : \N^n\to \N$ (e dunque l'identità $\pi_1:\N^1\to\N$ e la scelta di un elemento $k: \N^0\to\N$).
        \item Per ogni $g : \N^k\to \N$ in $R$ e $f_1,...,f_k : \N^n\to \N$ in $R$, la composizione (definita solo dove l'immagine delle $f_i$ appartiene al dominio di $g$) $g(f_1,...,f_k) : \N^n\to\N$ appartiene a $R$.
        \item Per $f:\N^k\to \N$ in $R$ e $g: \N^{k+2}$ in $R$ esiste un'unica $h:\N^{k+1}\to \N$ in $R$ tale che per ogni $\mathbf{x} \in \N^k$ e per ogni $y \in \N$ vale
        \[ h(\mathbf{x},0) = f(\mathbf{x})\qquad \text{e}\qquad h(\mathbf{x},y+1) = g(\mathbf{x},y,h(\mathbf{x},y))\]
        \item Per ogni $f : \N^{k+1}$ in $R$ esiste una funzione $\mu_f: \N^k \to \N$ in $R$ definita da 
        \[ \mu_f(\mathbf{x}) = \min\{y \in \N : f(\mathbf{x},y)=0\}\]
        dove questo minimo esiste (ovvero dove l'insieme non è vuoto), indefinita altrimenti.
    \end{enumerate}
    Definiamo $\R^*$ come il minimo insieme che soddisfa le condizioni da (1) a (5) e $\R$ come il minimo insieme che soddisfa le condizioni da (1) a (6).
\end{definition}

\begin{theorem}{}{}
    Le funzioni in $\R^*$ sono tutte e sole le funzioni calcolabili da un linguaggio di programmazione imperativo con le operazioni cicli di lunghezza finita e predeterminata.
    \proof 
    Dimostriamo che tutte le funzioni in $\R$ sono calcolabili per induzione scrivendo delle funzioni in Python, gli altri linguaggi imperativi sono equivalenti:
    Per l'implicazione inversa, consideriamo 
\end{theorem}

\chapter{Categorie Abeliane}
