...
\begin{definition}{Miscellaneous for morphisms}{miscmorphisms}
    Let $\mc{C}$ be a category.\begin{itemize}
        ...
        \item if $\hom\mc{C}$ is a set, $\mc{C}$ is said to be \bemph{small}, otherwise it is said to be \bemph{large}; if for all objects $x,y$, $\hom(x,y)$ is a set, $\mc{C}$ is said to be \bemph{locally small}.
