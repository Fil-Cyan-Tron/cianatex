...
\begin{theorem}{Lemma di Yoneda covariante}{covariant yoneda lemma}
    Sia $F:\C\to\setcat$ un funtore covariante. Allora esiste una biezione di insiemi\footnote{\PHcat\ Questi non sono insiemi! Una trasformazione naturale è una \textit{classe}, in generale una classe propria! È solo un caso che nel lato sinistro abbiamo una collezione "piccola" di oggetti "grandi", che però non è un insieme!}
    \[ \mc{Y}: \nat (h_A, F) \cong F(A)\]
