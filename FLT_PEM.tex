...
Con un background di teoria dei tipi è possibile ben fondare tutto quello che faremo, ma ciò esula dagli scopi di questa trattazione: procederemo dunque con una fede incrollabile e un ottimismo completamente ingiustificato, come sempre d'altronde.\\
...
\begin{remark}{}{}
    ...
    \end{itemize} 
    \proof 
    Dimostriamo solo l'ultimo punto con un esempio, gli altri sono banali. Sia $V$ la categoria formata da un unico oggetto $\bullet$ e la cui classe dei morfismi corrisponde alla classe dei cardinali, dove la composizione di due morfismi è data dalla loro somma come cardinali. Nonostante $\ob V$ sia la più piccola possibile, $\hom V$ è una classe propria, dunque $V$ non solo è grande, ma non è nemmeno localmente piccola.
\end{remark}
...
\begin{lemma}{Essenziale unicità degli aggiunti}{}
    ...
    Allora $G_1\cong G_2$.
    \begin{proof}
        Siano $\mu_{c,d} : \D(Fc, d) \to \C(c,G_1 d)$ e $\nu_{c,d} : \D(Fc, d) \to \C(c,G_2 d)$ gli isomorfismi naturali garantiti dalle aggiunzioni: ponendo $\lambda_{c,d} := \mu_{c,d}^{-1}\circ \nu_{c,d} : \C(c,G_1 d) \to \C(c,G_2 d)$ otteniamo un isomorfismo naturale
    \end{proof}
\end{lemma}

\subsection{Lemma e immersioni di Yoneda}
...
\begin{definition}{$\hom$-funtore covariante e controvariante}{hom-functor}
    Sia $\C$ una categoria e sia $A$ un oggetto di $\C$. Definiamo due funtori $h_A:\C\to\setcat$ e $h^A : \C^\op\to\setcat$, detti $\hom$\bemph{-funtori} (rispettivamente covariante e controvariante) nel seguente modo:
    \[\begin{tikzcd}
    	{\hom(A,X)} && X && {\hom(X,A)} \\
    	\\
    	{\hom(A,Y)} && Y && {\hom(Y,A)}
    	\arrow["{f\circ -}"{description}, from=1-1, to=3-1]
    	\arrow["{h_A}"{description}, from=1-3, to=1-1]
    	\arrow["{h^A}"{description}, from=1-3, to=1-5]
    	\arrow["f"{description}, from=1-3, to=3-3]
    	\arrow["{-\circ f}"{description}, from=3-5, to=1-5]
    	\arrow["{h_A}"{description}, from=3-3, to=3-1]
    	\arrow["{h^A}"{description}, from=3-3, to=3-5]
    \end{tikzcd}\]
...
\begin{theorem}{Lemma di Yoneda covariante}{covariant yoneda lemma}
    Sia $F:\C\to\setcat$ un funtore covariante. Allora esiste una biezione di insiemi \PHcat:
    \[ \nat (h_A, F) \cong F(A)\]
    e questa è naturale in $A$ e $F$.
    \proof 
    Sia $\Phi \in \nat(h_A,F)$. Dato che è naturale, il seguente diagramma commuta: 
    \[\begin{tikzcd}
    	A && {h_A(A)} &&&& {F(A)} \\
    	&&& {\id_A} && u \\
    	\\
    ...
    Vediamo che ci basta specificare l'assegnazione blu per determinare univocamente tutto il resto:\begin{itemize}
        \item Partendo da $\Phi \in \nat(h_A, F)$ ci basta specificare $\mc{Y}(\Phi) = u :=\Phi_A(\id_A)$ elemento di $F(A)$.
        \item Partendo da $u \in F(A)$ possiamo costruire a ritroso $\Phi := \mc{Y}^{-1}(u)$ scorrendo su $X \in \C$ e assegnando $\Phi_X(f:A\to X) := Ff(u)$.
    \end{itemize} 
    \qed
\end{theorem}
...
\begin{corollary}{Lemma di Yoneda controvariante}{contravariant yoneda lemma}
    ...
    e questa è naturale in $A$ e $F$.
    \proof 
    Analoga a quella del teorema precedente
\end{corollary}

\begin{remark}{\PHcat: Yoneda non ci dà una biezione di insiemi!}{}
    Questi non sono insiemi! Una trasformazione naturale è una \textit{classe}, in generale una classe propria! È solo un caso che nel lato sinistro abbiamo una collezione "piccola" di oggetti "grandi", che però non è un insieme! Vediamo un esempio:\\
    Consideriamo $\C := \grpcat$ la categoria dei gruppi e degli omomorfismi tra gruppi con il funtore dimenticante $U : (G, \cdot )\mapsto G$. Yoneda ci dice che:
    \[ \nat (h_G, U) \cong G \in \setcat \]
    Vediamo però che ogni trasformazione naturale $\Phi : h_G \to U$ corrisponde a una classe $\{\Phi_X\}_{X\in\grpcat}$, che è una classe propria! Quindi noi abbiamo una classe di classi che però abbiamo specificato \textit{non} poter essere membri di altre classi. La teoria delle categorie è falsa e da buttare in toto dunque, dato che persino uno dei suoi risultati più fondamentali è contradditorio?\\
    Ovviamente no, però ci mostra che la teoria degli insiemi di ZFC risulta inadeguata per trattare rigorosamente la teoria delle categorie (soprattutto quando si inizia a parlare di teoria delle categorie superiori, dove studiamo oggetti come la "metacategoria" $\CATcat$ delle categorie). Ci sono principalmente due approcci in qualche modo "duali" intrapresi da diversi autori:\begin{itemize}
        \item L'approccio "alla teoria dei tipi": abbandonare ZFC e fondare la teoria delle categorie su varie teorie dei tipi che permettano di trattare oggetti "arbitrariamente" più "grossi".
        \item L'approccio "alla teoria dei modelli": estendere ZFC con assiomi più o meno conservatori che permettano di "limitare" le delle categorie con degli opportuni cardinali.
    \end{itemize}
...
Una delle più naturali applicazioni del lemma di Yoneda è l'embedding di Yoneda, fondamentale in gran parte della topologia e della geometria moderna: esso permette di identificare una categoria con una sottocategoria 

\begin{theorem}{Embedding di Yoneda covariante}{covariant yoneda embedding}
    Sia $\C$ una categoria. Questa è equivalente ad una sottocategoria piena di $[\C^\op, \setcat]$ tramite la mappa $\mc{Y} : \C \to [\C^\op, \setcat]$ definita da:
    \[ \mc{Y}(f:A\to B) :=  (-\circ f):h^A\to h^B.\]
    \proof
    ...
    Vediamo che dunque l'assegnazione $A\mapsto h^A$ definisce un funtore covariante $h^\bullet : \C\to [\C^\op,\setcat]$ pienamente fedele (ma non essenzialmente suriettivo sugli oggetti). Restringendo $h^\bullet$ all'immagine di $\C$ in $[\C^\op,\setcat]$, otteniamo un'equivalenza di categorie (dato che la restrizione all'immagine è tautologicamente suriettiva).
    \qed
\end{theorem}

vediamo un esempio

\begin{example}{Prefascio delle funzioni continue}{}
    Sia $(X,\tau)$ uno spazio topologico. Consideriamo la sua topologia $\tau$ come una categoria piccola, dove una freccia $U\to V$ è data dall'inclusione insiemistica $U\subset V$. Per ogni aperto $U \in \tau$ consideriamo l'insieme $\C(U):=\C^0(U,\R)$ delle funzioni continue in $U$ a valore reale.\\
    Abbiamo che la mappa $U \mapsto \C(U)$, con l'assegnazione $(f : V\to \R) \mapsto (f|_{U\subset V} : U\to\R)$ è un funtore controvariante $\C : \tau^\op \to \setcat$, detto \bemph{prefascio delle funzioni continue} su $(X,\tau)$.
\end{example}
...
\begin{corollary}{Embedding di Yoneda controvariante}{contravariant yoneda embedding}
    Sia $\C$ una categoria. Allora la sua categoria opposta $\C^\op$ è equivalente ad una sottocategoria piena di $[\C, \setcat]$ tramite la mappa $\mc{Y}' : \C^\op \to [\C, \setcat]$ definita da:
    \[ \mc{Y}(f:A\to B) :=  (f\circ -):h_B\to h_A.\]
    \proof 
    Assolutamente analoga al teorema precedente.
    \qed
\end{corollary}
