...
\begin{definition}{Sottocategoria}{sottocategoria}
    ...
    \[h = f\circ_\C g \Harr h = f\circ_\D g.\]
...
Grazie al lemma \ref{lem:unicità degli aggiunti} abbiamo immediatamente che il prodotto è essenzialmente unico.

\subsection{Equivalenza}

\begin{theorem}{Equivalenza di definizioni}{equivalenza definizioni prodotti}
    Le definizioni \ref{def:prodotto pu}, \ref{def:prodotto coni} e \ref{def:prodotto adj} sono essenzialmente equivalenti.
    \proof 
    \begin{itemize}
        \item L'equivalenza di \ref{def:prodotto pu} e \ref{def:prodotto coni} è abbastanza evidente (in effetti si trattano entrambi di applicazioni del concetto di limite).
        \item \PHcat : dimostrare che \ref{def:prodotto pu} e \ref{def:prodotto adj} sono equivalenti può sembrare abbastanza immediato, ma c'è un piccolo ostacolo preliminare: noi non abbiamo un \textit{unico} oggetto prodotto e un \textit{unico} funtore prodotto, ma ne abbiamo potenzialmente delle classi di equivalenza: il fatto di prendere dei rappresentanti da esse per dimostrare che queste effettivamente fanno ciò che vogliamo richiede l'utilizzo di un assioma molto potente:
        \begin{axiom}{Scelta globale}{scelta globale}
            Sia $\V$ la classe di tutti gli insiemi.\\
            Esiste ed è non vuota la classe prodotto
            \[ \mfk{V}:= \prod_{S \in \V} S\]
        \end{axiom}
    \end{itemize}
\end{theorem}
