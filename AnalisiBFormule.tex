...
\begin{remark}{Prodotto vettoriale come determinante formale}{}
    Siano $v,w\in \R^3$, allora $v\wedge w$ si può calcolare come:
...
\begin{theorem}{Teorema di Gauss-Green nel piano}{}
    Sia $E \subset \R^2$ un insieme tale che $(\partial E, \tau)$ sia una curva regolare orientata, con vettore normale $\nu = -R\tau = (\tau_y, -\tau_x)$ e $F : \R^2 \to \R^2$ un campo vettoriale $\mc{C}^1$. Valgono:
    \[\int\limits_E \nabla\cdot F \de \L^2 = \int\limits_{\partial E}F \cdot \nu \de \H^1\]
    \[\int_{(\partial E, \tau)} F = \int_{\partial E} F \cdot \tau \de \H^1 = \int_E D_xF_y - D_yF_x \de \L^2\]
\end{theorem}
...
\begin{remark}{Rotore come determinante formale}{}
    Alternativamente, il rotore di in campo $F : \R^3 \to \R^3$ si può calcolare come il prodotto vettoriale di $\nabla$ con $F$ visto come determinante formale come sopra:
    \[\nabla \wedge F = \det \begin{pmatrix}
        \hat{i} && \hat{j} && \hat{k}\\
        D_x && D_y && D_z\\
        F_x && F_y && F_z
    \end{pmatrix}\]
