...
\title{Elementi di Teoria delle Categorie}
\author{Seminario PEM di F. Troncana, sotto la supervisione del prof. R. Zunino}
\date{A.A. 2025/2026}
...
\begin{abstract}
    ...
    In questa trattazione forniremo un'elementare esposizione del linguaggio e di alcune costruzioni e risultati fondamentali nella Teoria delle Categorie in modo propedeutico alla pratica matematica generale ed evidenziando alcuni aspetti di tipo fondativo.
\end{abstract}

\subsection*{Estensioni di ZFC}

Trattando strutture molto "grandi" in un senso che renderemo preciso in seguito, per fondare propriamente la Teoria delle Categorie è necessario estendere la Teoria degli Insiemi di Zermelo-Fraenkel più l'assioma della scelta (che assumeremo sempre). L'approccio che seguiremo è un approccio che in qualche modo "limiti" la grandezza delle nostre strutture, supponendo l'esistenza di un insieme "universo" che si sostituisca alla classe di tutti gli insiemi.
...
Da queste proprietà seguono due fatti strettamente correlati, ovvero che un universo di Grothendieck più che numerabile è un \bemph{modello} per ZFC e che l'esistenza di un universo di Grothendieck più che numerabile è indipendente da ZFC. Intuitivamente, il seguente assioma ci permette di trattare un universo di Grothendieck come "sostituto" della classe di tutti gli insiemi, sicuri del fatto che questo "limiti" tutta la pratica matematica "usuale".

\begin{axiom}{Esistenza di un universo di Grothendieck più che numerabile}{esistenza di un universo di Grothendieck più che numerabile}
    Esiste un universo di Grothendieck più che numerabile $\G$. Tutti gli elementi di $\G$ si dicono \bemph{piccoli}, mentre gli altri insiemi si dicono \bemph{grandi}.
\end{axiom}
...
\begin{definition}{Categoria e dualità}{categoria}
    ...
    Una categoria $\C$ si dice:\begin{itemize}
        \item \bemph{Piccola} se gli insiemi $\hom\C$ e $\ob\C$ (anche se vedremo sotto che la grandezza della seconda è sempre limitata dalla prima) sono insiemi piccoli.
...
\begin{remark}{Sulle categorie}{grandezza}
    ...
    \end{itemize} 
    \proof 
    Dimostriamo solo l'ultimo punto con un esempio, gli altri sono banali. Sia $V$ la categoria formata da un unico oggetto $\bullet$ e il cui insieme dei morfismi corrisponde all'insieme dei cardinali minori della cardinalità di $\G$, dove la composizione di due morfismi è data dalla loro somma come cardinali. Nonostante $\ob V$ sia il più piccolo possibile (al di là della categoria vuota), $\hom V$ è un insieme grande, dunque $V$ non solo è grande, ma non è nemmeno localmente piccola.
    \qed
\end{remark}

Da ora in avanti, assumeremo sempre (anche senza specificarlo) che le nostre categorie siano almeno localmente piccole per evitare problemi di fondazione.\\
Inoltre quando faremo riferimento ad alcune categorie specifiche, ad esempio $\setcat$, assumeremo sempre di intendere non la categoria di tutti gli insiemi con le funzioni tra di loro\footnote{Che non sarebbe affatto una categoria per come le abbiamo definite} ma della categoria di tutti gli insiemi piccoli con le funzioni tra di loro\footnote{Possiamo farlo in quanto $\G$ contiene automaticamente tutti gli insiemi costruiti a partire dall'insieme vuoto e iterando operazioni di unione, intersezione e passaggio alle parti, dunque di fatto tutti gli insiemi che si usano nella normale pratica matematica}, e allo stesso modo intenderemo per esempio gli spazi topologici piccoli, o i gruppi piccoli e così via.
...
\begin{remark}{Lemma di Yoneda e grandezza}{lemma di Yoneda e grandezza}
    In generale la biezione dataci da \ref{th:lemma di yoneda covariante} non è una biezione tra insiemi piccoli, in quanto gli elementi di $\nat(h_A,F)$ non sono in generale insiemi piccoli. Prendiamo ad esempio il funtore dimenticante $U$ da $\grpcat$\footnote{Categoria dei gruppi con gli omomorfismi di gruppo} a $\setcat$, che ad un gruppo $(G,\cdot)$ associa l'insieme $G$: noi abbiamo che $\nat(h_G,U)$ è in biezione con $G$, ma gli elementi di $\nat(h_G,U)$ sono indicizzati dall'insieme di tutti i gruppi (piccoli), che non è un insieme piccolo e dunque questi non sono insiemi piccoli, nonostante l'insieme che li contenga sia in biezione con un insieme piccolo.
\end{remark}
