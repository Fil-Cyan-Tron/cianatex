...
\begin{remark}{Lemma di Yoneda e grandezza}{lemma di Yoneda e grandezza}
    In generale l'isomorfismo datoci da \ref{th:lemma di yoneda covariante} non è una biezione tra insiemi piccoli, in quanto gli elementi di $\nat(h_A,F)$ non sono in generale insiemi piccoli.\\
    ...
    Avendo l'assioma \ref{ax:assioma di universo} non incorriamo in problemi di tipo logico o fondativo, ma se non avessimo limitato in questo modo la grandezza delle nostre categorie dovremmo gestire in qualche modo la collezione $\nat(h_S,\mathbf{Id})$, che per quanto ci sia garantito debba essere "grande quanto" un insieme, sarebbe una collezione di elementi normalmente considerati come classi proprie, ovvero classi che non possono appartenere ad altre classi.
\end{remark}
...
\begin{lemma}{Essenziale unicità degli aggiunti}{unicità degli aggiunti}
    ...
    Ottenendo dunque $\mc{Y}\circ G_1 \cong \mc{Y}\circ G_2$: dal teorema \ref{th:rappresentazione di yoneda covariante}, ovvero la piena fedeltà della rappresentazione di Yoneda, segue la tesi.
    \qed
\end{lemma}
...
\begin{example}{$R\modcat$ non è cartesiana chiusa}{rmod non è ccc}
    ...
    \proof 
    $R\modcat$ ha un oggetto zero $\{0\}$, il modulo banale, ma dunque se fosse cartesiana chiusa tutti i moduli sarebbero isomorfi per il lemma \ref{lem:ccc e oggetti zero}; dato che questo non è vero, $R\modcat$ non può essere cartesiana chiusa.
    \qed
\end{example}
...
\begin{theorem}{Tesi di Church-Turing per $\imp$}{}
    ...
    Dimostriamo per induzione che le funzioni in $\hom\grf(\N)$ sono calcolabili da $\stimp$; specificheremo sempre il tipo delle espressioni coinvolte:\begin{enumerate}
        ...
        \end{enumerate}
        La formulazione (a) è più in linea con la definizione che abbiamo dato in \ref{def:grf e prf}, mentre la formulazione (b) rende più evidente il fatto che sia calcolabile da un ciclo di lunghezza predeterminata.
...
\begin{remark}{}{}
    L'usuale definizione di funzione ricorsiva primitiva o generale è leggermente diversa da quella che abbiamo dato in \ref{def:grf e prf}: generalmente questa viene data molto "con le mani" per funzioni $\N^n\to\N$, intendendo una funzione $\N^n\to\N^m$ come una $m$-upla di funzioni $\N^n\to\N$
\end{remark}
