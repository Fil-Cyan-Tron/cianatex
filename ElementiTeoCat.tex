...
Trattando strutture molto "grandi" in un senso che renderemo preciso in seguito, per fondare propriamente la Teoria delle Categorie è necessario estendere la Teoria degli Insiemi di Zermelo-Fraenkel più l'assioma della scelta, che assumeremo sempre\footnote{Potremmo essere tentati di limitare l'assioma della scelta a famiglie piccole o almeno moderate di insiemi (nel senso specificato in \cref{ax:assioma di universo}), ma ai fini della nostra trattazione lo assumeremo sempre nella sua forma completa.}. L'approccio che seguiremo è un approccio che in qualche modo "limiti" la grandezza delle nostre strutture, supponendo l'esistenza di alcuni insiemi "universo" che si sostituiscano alla classe di tutti gli insiemi.\\
Questa sezione è quasi interamente basata sull'\textit{expose} I.0 di \cite{SGA4}. Assumeremo inoltre che ogni insieme sia un insieme puro.
...
In \cite{MacLane1978} viene data una definizione meno estensiva di grandezza, supponendo di trovarsi in un qualche universo lasciandosi la libertà di "ingrandirlo" rimpiazzandolo con uno maggiore laddove ci si trovasse a trattare categorie troppo grandi. Qui abbiamo semplicemente qualificato e chiarito meglio l'operazione di scelta di universi più grandi.

\begin{remark}{Su $\{\G_\alpha\}_{\alpha}$}{insiemi alpha-piccoli, modelli di ZFC}
    L'insieme $\G_0$ è numerabile e corrisponde alla classe degli insiemi ereditariamente finiti, ovvero degli insiemi finiti i cui elementi sono insiemi ereditariamente finiti.\\
    Gli insiemi $\alpha$-moderati sono $\alpha+1$-piccoli.\\
\end{remark}
...
Vale inoltre un teorema molto importante che stabilisce una corrispondenza tra universi di Grothendieck e cardinali fortemente inaccessibili in ZFC:

\begin{theorem}{Universi di Grothendieck e cardinali inaccessibili}{}
    Sia $U$ un universo di Grothendieck più che numerabile.\\
    Il cardinale $|U|$ è fortemente inaccessibile.
\end{theorem}
...
\begin{remark}{Sulle categorie}{grandezza}
    ...
    \end{itemize} 
    \proof 
    Forniamo un esempio dell'ultimo punto, gli altri sono banali.\\
    Sia $\V$ la categoria con $\ob\V := \{\varnothing\}$ e $(\hom\V, \circ) = (\G_\omega,\cup)$: l'insieme degli oggetti di $\V$ è il più piccolo non vuoto possibile, mentre l'insieme dei morfismi di $\V$ è un insieme grande, dunque $\V$ non solo è grande ma non è nemmeno localmente piccola.
    \qed
\end{remark}
...
\begin{remark}{Sui morfismi}{sui morfismi}
    ...
    Forniamo solo due esempi di morfismi che sono epici e monici ma non isomorfismi (le altre verifiche sono assolutamente automatiche):\begin{itemize}
        ...
        \end{tikzcd}\]
        Dato che a destra o sinistra possiamo comporre solo con l'identità, $1\le 2$ è sia monico che epico, ma non è isomorfismo in quanto non ha inverso.
    \end{itemize}
    \qed
\end{remark}
...
\begin{definition}{Oggetti iniziali e terminali}{Oggetti iniziali e terminali}
    ...
    $I$ si dice \bemph{oggetto terminale} se è l'oggetto iniziale di $\C^\op$, o equivalentemente se per ogni oggetto $X$ di $\C$ esiste un unico morfismo $\zeta_X : X\to I$.\\
...
\begin{proposition}{Unicità di oggetti iniziali e terminali}{unicità zero}
    ...
    \proof 
    Siano $I, I'$ oggetti iniziali di $\C$; per definizione di oggetto iniziale, per ogni oggetto $X$ di $\C$ deve valere $\iota_X = \iota'_X \circ \iota_{I'}$, dunque ponendo $X=I$ otteniamo $\iota_I = \id_I = \iota'_I\circ \iota_{I'}$; analogamente otteniamo l'altro lato dell'inversione.
    \qed
\end{proposition}
...
Gli $\hom$-funtori sono l'esempio prototipico di \bemph{endoprofuntori}: un profuntore da $\C$ a $\D$ è un funtore $\D^\op\times\C\to\setcat$, 
