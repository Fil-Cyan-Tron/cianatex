...
\begin{axiom}{Assioma di Universo}{assioma di universo}
    ...
    \[ \G_\omega := \min_{\subset}\left\{ U \in \mathbb{G} : \bigcup_{\alpha < \omega} \G_\alpha \in U\right\} = \bigcap\left\{ U \in \mathbb{G} : \forall \alpha < \omega, \G_\alpha \in U \right\} \]
    ...
    Diremo in generale \bemph{piccoli} gli insiemi che sono $\alpha$-piccoli per un qualche $\alpha <\omega$, \bemph{moderati} gli insiemi $\omega$-moderati e \bemph{grandi} gli insiemi $\omega$-grandi.
\end{axiom}

La costruzione di $\G_\lambda$ può essere facilmente generalizzata a qualsiasi ordinale $\lambda$ sia nel caso di un ordinale successore che nel caso di un ordinale limite, ma per i nostri scopi è sufficiente fermarci al primo ordinale infinito.

\begin{remark}{Su $\{\G_\alpha\}_{\alpha \in \omega}$}{insiemi alpha-piccoli, modelli di ZFC}
    ...
    Gli insiemi $\alpha$-moderati per un qualche $\alpha <\omega$ sono $\alpha+1$-piccoli.\\
    $\{\G_\omega\}_{\alpha \le \omega}$ è ben ordinata per inclusione.
\end{remark}

Intuitivamente, $\G_1$ è già sufficiente per la pratica matematica "ordinaria", contenendo tutte le costruzioni usuali, e appunto è già un candidato per la "classe di tutti gli insiemi" in ZFC costituendone un modello. 

\begin{theorem}{Modelli per ZFC in $\{\G_\alpha\}_{\alpha\le\omega}$}{modelli per ZFC}
    L'esistenza di $\G_0$ è indipendente dalla teoria degli insiemi di ZF meno l'assioma di infinito e questo ne costituisce un modello; in particolare, l'esistenza di $\G_0$ in questa teoria è equivalente all'assioma di infinito.\\
...
In \cite{MacLane1978} viene data una definizione meno estensiva di grandezza, assumendo l'esistenza di un solo universo per la trattazione generale e lasciando al lettore la libertà di "ingrandirlo" rimpiazzandolo con uno maggiore laddove questi si trovasse a trattare categorie troppo grandi. Qui abbiamo semplicemente qualificato e chiarito meglio l'operazione di scelta di universi più grandi: ci siamo limitati ad una gerarchia di $\omega$ universi, ma nulla impedisce di estendere la stessa operazione a ordinali arbitrari.
...
\begin{remark}{Sulle categorie}{grandezza}
    ...
    Sia $\V$ la categoria con $\ob\V := \{\varnothing\}$ e $(\hom\V, \circ) \cong (\G_\omega,\cup)$: l'insieme degli oggetti di $\V$ è il più piccolo non vuoto possibile, mentre l'insieme dei morfismi di $\V$ è un insieme grande, dunque $\V$ non è solo grande, ma non è nemmeno localmente piccola.
    \qed
\end{remark}
...
\begin{example}{La categoria degli insiemi ($\alpha$-)piccoli e categorie concrete}{setcat e categorie concrete}
    Per $\alpha \le \omega$, indichiamo con $\setcat_\alpha$ la categoria i cui oggetti sono insiemi $\alpha$-piccoli, ovvero elementi di $\G_\alpha$, e i cui morfismi sono le funzioni tra di loro con l'usuale composizione di funzioni insiemistiche; indicheremo semplicemente con $\setcat$ la categoria $\setcat_\omega$.\\
    ...
    \[ \hom\left(1,\bigcup_{\alpha < \omega}\G_\alpha\right), \qquad \hom\left(\bigcup_{\alpha < \omega}\G_\alpha, 2 \right)\]
    entrambi con cardinalità strettamente maggiore di qualsiasi $\G_\alpha$.\\
    Per qualsiasi $\alpha \le \omega$, $\setcat_\alpha$ ha un unico oggetto iniziale, l'insieme vuoto $0$, ed essenzialmente un'unico oggetto terminale, l'insieme $1$.
\end{example}
...
\begin{definition}{Funtori aggiunti}{funtori aggiunti}
    Siano $\C,\D$ due categorie localmente piccole e $F:\C\to\D$ e $G:\D\to\C$ due funtori.\\
    Questi si dicono \bemph{aggiunti} se esiste un isomorfismo in $\setcat$
    \[ \D(Fc, d) \cong \C(c, Gd)\]
