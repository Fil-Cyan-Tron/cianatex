...
\begin{example}{$\setcat_\alpha$ è cartesiana chiusa}{set è ccc}
    ...
    Controlliamo le tre condizioni:\begin{enumerate}
        ...
        \[\begin{tikzcd}
        	...
        	{Z^Y\times Y} &&&& Z
        	\arrow["f"{description}, from=1-2, to=3-5]
        	\arrow[equals, from=2-1, to=1-2]
        	\arrow["{\lambda f\times \id_Y}"{description}, from=2-1, to=5-1]
        	\arrow[maps to, from=2-2, to=4-2]
...
\begin{definition}{}{}
    ...
    I morfismi identità corrispondono alle funzioni identità e la composizione $g\circ f$ è data dalla funzione $g|_{\dom f} \circ f$
\end{definition}

\begin{remark}{}{}
    Per ogni $n,m \in \N$, la funzione vuota $\varnothing$ definisce un valido morfismo $\N^n\to \N^m$.
\end{remark}
...
\begin{definition}{}{grf e prf}
    Consideriamo la sottocategoria $\grf(\N)$ di $\pf(\N)$ definita da $\ob\grf(\N) = \ob\pf(\N)$ e il cui insieme dei morfismi $\R$ è definito come il minimo sottoinsieme di $\hom\pf(\N)$ tale che:\begin{enumerate}
        ...
        \item Le proiezioni $\pi_i : \N^n \to \N$ appartengono a $\R$; notiamo che questo include la funzione $\pi_0: 1\to \N$ di scelta di un elemento.
    ...
    \end{enumerate}
    Questa è detta categoria delle \bemph{funzioni ricorsive generali}; omettendo il requisito $6$ si ottiene la categoria delle \bemph{funzioni ricorsive primitive} $\prf(\N)$.
\end{definition}

\newcommand\imp{\catname{IMP}}
\newcommand\stimp{\imp^{(\to)}}
\newcommand\T{\mathcal{T}}
\subsection{Il linguaggio $\stimp$}

Definiamo un linguaggio di programmazione "giocattolo" che chiameremo $\stimp$, o $\imp$ semplicemente tipato, ma che dimostreremo essere sufficientemente potente da calcolare tutte (e sole) le funzioni in $\hom\grf(\N)$. È basato sul linguaggio $\imp$ definito in CITAZUN, ma permettiamo che le variabili attingano da un sistema di tipi più ampio:

\begin{definition}{Sistema di tipi e sintassi di $\stimp$}{}
    Sia $\Gamma$ un alfabeto e $^*$ la stella di Kleene, definiamo il sistema di tipi $\T$ per induzione tramite le seguenti regole:
    \[ \frac{}{\Gamma^*}\qquad \frac{}{\N} \qquad \frac{\N^n}{\N^{n+1}}(n \in \N)\qquad \frac{\tau \qquad \sigma}{\tau \to \sigma} \]
    Per indicare che un termine $x$ ha tipo $T$, scriveremo $x:T$.\\
    Estendiamo la sintassi delle espressioni per la valutazione di funzioni, la concatenazione di stringhe:
    \[ \frac{e:S\quad f:S\to T }{f(e):T}\qquad \frac{e_1:\Gamma^*\quad e_2:\Gamma^*}{e_1e_2 : \Gamma^*} \]
...
\begin{theorem}{Tesi di Church-Turing per $\stimp$}{}
    Una funzione $f:\N^m\to\N^n$ appartiene a $\hom\grf(\N)$ se e solo se esiste un programma di $\stimp$ che la calcola.\\
    Una funzione $f:\N^m\to\N^n$ appartiene a $\hom\prf(\N)$ se e solo se esiste un programma di $\stimp$ che la calcola con cicli while di lunghezza al più predeterminata.
    \proof 
    Dimostriamo per induzione che le funzioni in $\hom\grf(\N)$ sono calcolabili da $\stimp$; specificheremo sempre il tipo delle espressioni coinvolte:\begin{enumerate}
        \item Sia $\sigma(x):\N^n$ per un qualche $n \in \N$; la funzione zero viene calcolata da
            \[ \texttt{Zero}(x:\N^n) = \left(x := 0:\N \right) \]
        \item Sia $\sigma(x):\N$; la funzione successore viene calcolata dal programma
            \[ \texttt{Succ}(x:\N) = \left( x:= x+1:\N \right) \]
        \item Siano $\sigma(x):\N$, $\sigma(X):\N^n$ per qualche $n \in \N$ e sia $\sigma(i):\N$ tale che $\sigma(i)<n$; la funzione proiezione viene calcolata dal programma
            \[ \texttt{Proj}(X:\N^n, i:\N) = \left( x := X(i):\N \right) \]
        \item \begin{verbatim}
            def comp(g, f, x):
                return g(f(x))
        \end{verbatim}
        La composizione di due funzioni calcolabili $f:\N^l\to\N^m$ e $g:\N^m\to\N^n$ è semplicemente la loro composizione come programmi $\texttt{F}$ e $\texttt{G}$
            \[ \texttt{G}\circ\texttt{F}(x:\N^l)  = \left(\texttt{F}(x:\N^l);\texttt{G}(x:\N^m) : \N^n\right) \]
        \item Abbiamo due versioni equivalenti, la prima è più elegante mentre la seconda mostra come venga rispettato il requisito di avere loop di lunghezza predeterminata:\begin{enumerate}
            \item \begin{verbatim}
                def ric(g, f, x, y):
                    if y == 0:
                        return f(x)
            ...
            \end{verbatim}
            \[ \texttt{H}(\texttt{G}(a:\N^n,b:\N,c:\N):\N, \texttt{F}(r):\N^n\to\N, x:\N^n, y:\N ) = \left( \text{if}\ y=0\ \text{then}\ \texttt{F}(x)\ \text{else}  \right) \]
            \item \begin{verbatim}
                def ric(g, f, x, y):
                    values = [f(x)]
...
\begin{definition}{Enumerazione standard di $\hom\grf(\N)$}{enumerazione standard}
    Fissiamo una biezione $\Phi : \N \to \hom\grf(\N)$ che diremo \bemph{enumerazione standard} di $\hom\grf(\N)$.\\
    Indicheremo con $\phi_i(x)$ la funzione $\Phi(i)(x)$
\end{definition}

\begin{theorem}{Teorema $s-m-n$}{}
    Sia $\phi_i: \N^{n+m} \to \N$ in $\hom\grf(\N)$.\\
    Esiste una funzione $s_m^n: \N^{1+n} \to \N$ in $\hom\prf(\N)$ tale che per ogni $\mathbf{x} \in \N^n$ e $\mathbf{y}\in \N^m$ si abbia
    \[ \phi_i(\mathbf{x},\mathbf{y}) = \phi_{s_m^n(i,\mathbf{x})}(\mathbf{y})\]
    In più, $s_m^n$ è univocamente determinata da $m$ e $n$
    \proof 
...
\begin{definition}{La categoria $\percat(\N)$}{}
    Definiamo la categoria $\percat(\N)$ come la categoria i cui oggetti sono le relazioni di equivalenza parziali su $\N$ identificate con i loro quozienti $\N/R$ e i cui morfismi sono dati dalle mappe $f:\N/R\to\N/S$ "tracciate" da un qualche $i \in \N$, ovvero da una mappa $\varphi_i \in \hom_\grf(\N,\N)$ definita sul dominio di $R$ tale che 
...
\begin{remark}{}{}
    Nella definizione dei morfismi di $\percat(\N)$ non è richiesto nè che $i$ sia unico nè che $\phi_i$ si comporti in qualsiasi modo prestabilito nella "zona d'ombra" di $R$
\end{remark}

\begin{lemma}{}{}
    Ogni relazione $T$ con un'unica classe di equivalenza non vuota $\dom T$ è un oggetto terminale di $\percat(\N)$.
    \proof 
    Prendiamo $T := \N^2$ e notiamo che per qualsiasi  funzione costante $\phi_i$ traccia un unico morfismo $R\to T$
\end{lemma}
