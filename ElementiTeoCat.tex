...
Questa sezione è interamente basata sull'\textit{expose} I.0 di \cite{SGA4}.
...
\begin{proposition}{Proprietà degli universi di Grothendieck}{proprietà degli universi di Grothendieck più che numerabili}
    Sia $U$ un universo di Grothendieck. Allora questo contiene:\begin{itemize}
...
\begin{axiom}{Assioma di Universo}{assioma di universo}
    ...
    Consideriamo nella classe $\mathbb{G}$ degli universi di Grothendieck\footnote{Ben ordinata per inclusione, come visto in \ref{prop:proprietà degli universi di Grothendieck più che numerabili}} la famiglia $\{\G_\alpha\}_{\alpha \le \omega}$, dove $\omega$ è il più piccolo ordinale infinito, definita come segue:
    \[ \G_0 := \min \mathbb{G}=\bigcap\mathbb{G} \]
    ...
    Diremo in generale \bemph{piccoli} gli insiemi $\alpha$-piccoli per un qualche $\alpha <\omega$, \bemph{moderati} gli insiemi $\omega$-moderati e \bemph{grandi} gli insiemi $\omega$-grandi.
\end{axiom}

\begin{remark}{Sugli insiemi $\alpha$-piccoli}{insiemi alpha-piccoli, modelli di ZFC}
...
\begin{theorem}{Modelli per ZFC in $\{\G_\alpha\}_{\alpha\le\omega}$}{modelli per ZFC}
    ...
    La relazione di appartenenza $\in$ rende ogni $\G_\alpha$ ben ordinato.
\end{theorem}
