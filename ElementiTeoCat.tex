...
\begin{definition}{}{}
    Sia $\pfn(\N)$ la categoria i cui oggetti sono potenze finite di $\N$, compreso $\N^0 = 1$, e i cui morfismi sono le \bemph{funzioni parziali} su $\N$, ovvero dove un morfismo $f:\N^n\to \N^m$ è dato da una funzione $f: D\to\N^m$, dove $D\subset \N^k$ può essere anche vuoto.\\
...
\begin{remark}{}{}
    Mentre l'insieme $\ob\pfn(\N)$ è numerabile, $\hom\pfn(\N)$ ha cardinalità del continuo.
    \proof 
    ...
    \[ \hom_\setcat(\N^n, \N^m) \subset \hom_\pfn(\N^n,\N^m) \subset \mc{P}(\N^n\times \N^m) \]
    Dunque passando alle cardinalità per $n,m \ge 1$ possiamo concludere che $\hom_\pfn(\N^n,\N^m)$ abbia cardinalità del continuo; a questo punto basta osservare che
    \[ \hom\pfn(\N) = \bigcup_{n\in \N, m\in \N} \hom_\pfn(\N^n,\N^m) \]
...
\begin{definition}{}{grf e prf}
    Consideriamo la sottocategoria $\grf(\N)$ di $\pfn(\N)$ definita da $\ob\grf(\N) = \ob\pfn(\N)$ e il cui insieme dei morfismi $\R$ è definito come il minimo sottoinsieme di $\hom\pfn(\N)$ tale che:\begin{enumerate}
        ...
        \end{cases}\]
        appartiene a $\R$.
        \item Se $f\in \hom_{\pfn}(\N^{k+1},\N)$, allora la funzione definita da
...
\begin{theorem}{Tesi di Church-Turing}{}
    L'insieme $\hom\grf(\N)$ è esattamente l'insieme delle funzioni calcolabili da un linguaggio di programmazione imperativo Turing-completo.\\
    L'insieme $\hom\prf(\N)$ è esattamente l'insieme delle funzioni calcolabili da un linguaggio di programmazione imperativo Turing-completo ammettendo loop di lunghezza predeterminata.
    \proof 
    Dimostriamo per induzione che le funzioni in $\hom\grf(\N)$ sono calcolabili da python; assumeremo che tutti i parametri passati siano come specificati in \ref{def:grf e prf}:\begin{enumerate}
        \item \begin{verbatim}
            def zero(x):
                return 0
        \end{verbatim}
