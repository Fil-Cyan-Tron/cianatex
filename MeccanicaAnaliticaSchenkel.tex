...
\begin{definition}{Vincoli olonomi}{vincoli olonomi}
    Un sistema di $0\le C\le N$ \bemph{vincoli olonomi} per un sistema di di $N\ge 1$ punti materiali è dato da una famiglia di $C$ funzioni liscie $f_j : \V^{3N+1}\to\R$ che soddisfano le seguenti proprietà:\begin{itemize}
        ...
        \[ \left[ \frac{\partial f_j}{\partial x^i} \bigg|_{(t,\vec{\vec x})} \right]_{1\le i\le 3N,1\le j\le C} : \R^{3N}\to \R^C \]
        Ha rango massimo per ogni $(t,\vec{\vec x})$ tale che $f_j((t,\vec{\vec x})) = 0$ per ogni $j\le C$
    \end{itemize}
...
\begin{proposition}{}{utile sulle varietà}
    ...
    \[\left[ \frac{\partial f_j}{\partial x^i} \bigg|_{(t,\vec{\vec x})} \right]_{0\le i\le 3N,1\le j\le C} : \R^{3N+1}\to \R^C\]
    questa matrice ha ovviamente rango massimo, perchè restringendoci a vettori della forma $(0,\vec{\vec{x}})$ otteniamo la matrice jacobiana non estesa, che ha rango massimo per definizione.\\
...
\begin{example}{}{}
    ...
    \[ f(t,\vec x) = (x^1 - O^1(t))^2 + (x^2 -O^2(t))^2 + (x^3 -O^3(t))^2 - r_0^2(t) \]
...
\begin{proposition}{}{}
    ...
    Poichè la jacobiana di ogni diffeomeorfismo è invertibile, ne segue che gli spazi tangenti coincidono.\\
...
\[ v|_R(t) := \left((\di R|_{\Sigma_t}^{\times N})^{-1}\circ \frac{\di}{\di t} \circ R|_{\Sigma_t}^{\times N} \right)(\Gamma(t)) = (v_{Q_1}|R(t),..., v_{Q_N}|_R(t)) \in V_t^\N \]
\[ a|_R(t) := \left((\di R|_{\Sigma_t}^{\times N})^{-1}\circ \frac{\di^2}{\di t^2} \circ R|_{\Sigma_t}^{\times N} \right)(\Gamma(t)) = (a_{Q_1}|R(t),..., a_{Q_N}|_R(t)) \in V_t^\N \]
